...
def welcomepage():
    ...
    st.markdown(
    f'<div style="text-align: center;"><img src="{info.profile_picture}" width="400"></div>',
...
def overview():
    ...
    if 'option' not in st.session_state:
        st.session_state.option = 'Land'
    
    option = st.radio( #NEW
        "Click to learn more about the geography of Turks and Caicos!",
        ("Land", "Water"),
        index=("Land", "Water").index(st.session_state.option)
    )
...
def static_chart():
    data = datal()
    st.write("Description: Some of my favorite vacation locations and the amount of times I have traveled there!")
...
def packing_slider():
    st.markdown('<h1 style="text-align: center;">Packing for Turks and Caicos</h1>', unsafe_allow_html=True)
    if 'num_items' not in st.session_state:
        st.session_state.num_items = 12
    st.markdown('<p style="font-size:20px; text-align: center;">How many outfits do you think I usually pack for a 5-day trip to Turks and Caicos?</p>', unsafe_allow_html=True)
...
def dynamic_chart():
    data = datal()
    st.markdown(
    '<h3 style="font-size: 30px; text-align: center;">If you think the amount of times I have been to Turks is bad, change the year to see all of the vacations I have taken within the past three years!</h3>',
    unsafe_allow_html=True
    )
    st.write("")
    st.write("")
    st.write("")
    st.markdown(
    '<div style="text-align: center;">PSA: Most of the trips in the year 2025 are trips that are planned for later this year!</div>',
    unsafe_allow_html=True
    )
    st.write("")
    st.write("")
...
def favorite_activities_pie():
    st.markdown(
    '<p style="font-size: 24px; text-align: center">I think after that graph you can truly imagine how much I love being in Turks! You must be wondering whats so fun about Turks and Caicos theres just beach but here is a pie chart that shows all of my favorite activities distributed based on how fun I think each is</p>',
    unsafe_allow_html=True
    )
    data = datal()
...
st.markdown(
    '<p style="font-size: 24px; text-align: center;">Hopefully now you can understand why I love Turks and Caicos so much! I hope you learned something new today and enjoyed reading about my favorite place to travel!</p>',
    unsafe_allow_html=True
)
